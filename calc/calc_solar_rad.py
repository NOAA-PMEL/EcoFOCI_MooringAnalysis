...
def incoming_solar_rad(time, lat, lon, sol0=1000.):
    """

    sol0 - W*m^-2
    """
    sol_time = Pysolar.GetSolarTime(lon,datetime.datetime.fromordinal(int(time))+datetime.timedelta(0,time-int(time))*86400)

    doy = datetime.datetime.fromordinal(int(time)).timetuple().tm_yday 
    solar_dec = Pysolar.GetDeclination(doy) #deg
    hour_dec = 15 * ( sol_time - 12.)

    zenith_angle = (np.sin(np.deg2rad(lat)) * np.sin(np.deg2rad(solar_dec)) + np.cos(np.deg2rad(lat)) * np.cos(np.deg2rad(solar_dec)) * np.cos(np.deg2rad(hour_dec)))
