...
def wind2vector(ws, wd):
    ...
    uu = -1.0 * ws * np.sin(wd * np.pi / 180.0)
    vv = -1.0 * ws * np.cos(wd * np.pi / 180.0)
...
def vector2wind(u, v, firstdefinition=False):
    ...
    ws = np.sqrt(u * u + v * v)
    if firstdefinition:
        wd = 270.0 - np.rad2deg(np.arctan2(v, u))
        wd[np.where(wd < 0)] = wd[np.where(wd < 0)] + 360.0
    else:
...
def latlon_dm2dd(Mooring_Lat, Mooring_Lon, pos_dir="W"):
    ...
    lat = float(Mooring_Lat[0]) + float(Mooring_Lat[1]) / 60.0

    lon = float(Mooring_Lon[0]) + float(Mooring_Lon[1]) / 60.0
    if pos_dir == "E":
        lon = -1 * lon
...
parser = argparse.ArgumentParser(
    description="Convert raw data from variety of instruments to netcdf archive format"
)
parser.add_argument(
    "DataFile", metavar="DataFile", type=str, help="full path to data file"
)
parser.add_argument(
...
parser.add_argument(
    "InstDepth", metavar="InstDepth", type=int, help="Nominal Instrument Depth"
)
parser.add_argument(
    "-conv", "--convention", type=str, help="EPIC or CF/COARDS format", default="epic"
)
parser.add_argument(
    "-ih",
    "--InstTypeHelp",
...
parser.add_argument(
    "-add_meta", "--add_meta", nargs=3, type=str, help="MooringID serial_no water_depth"
)
...
if args.InstType in ["MTR", "mtr"]:
...
elif args.InstType in ["sbe26", "sbe-26", "SBE26", "SBE-26", "s26"]:
    config_file = instr_data_ingest.data_source_instrumentconfig("yaml").get(
        args.InstType
    )
    Dataset = instr_data_ingest.get_inst_data(
        args.DataFile, source=args.InstType, round_quarter_hour=True
    )
    ...
    try:
        ...
        data_dic["P_1"] = (data_dic["P_1"] - (atm_sealevel / 10.0)) / 10.0
    except:
        data_dic["P_1"] = np.ones_like(Dataset["time"].values()) * 1e35
    try:
    ...
    if args.declination:
        ...
        new_history = True
        history_string = "Magnetic Declination Correction {dec} applied (+E)".format(
            dec=dec
        )
    else:
        (lat, lon) = (-9999, -9999)
    ...
    Dataset = instr_data_ingest.get_inst_data(
        args.DataFile, source=args.InstType, argos_file=to_bool(args.keywordargs[0])
    )
    if (args.convention).upper() in ["epic", "EPIC"]:
        EPIC_VARS_dict = get_config(configPath + config_file, "yaml")
    else:
    ...
    if args.declination:
        ...
        data_dic["WU_422"], data_dic["WV_423"] = rotate_coord(
            wu, wv, declination_corr=dec
        )
        data_dic["WS_401"], data_dic["WD_410"] = vector2wind(
            data_dic["WU_422"], data_dic["WV_423"]
        )
    ...
    Dataset = instr_data_ingest.get_inst_data(
        args.DataFile, source=args.InstType, roundTime=to_bool(args.keywordargs[0])
    )
...
if (args.convention).upper() in ["epic", "EPIC"]:
    ...
    ncinstance.sbeglobal_atts(
        raw_data_file=args.DataFile.split("/")[-1],
    ...
    ncinstance.add_coord_data(
        depth=args.InstDepth, latitude=lat, longitude=lon, time1=time1, time2=time2
    )
    ...
    ncinstance.variable_init(EPIC_VARS_dict)
    ncinstance.add_coord_data(
        depth=args.InstDepth, latitude=lat, longitude=lon, time=time_cf
    )
