...
def is_in_dic(var, dic):
    for cmd_var in var:
        if not cmd_var in dic.keys():
            print(f"Epic Key {cmd_var} not in data file... exiting")
            sys.exit()
...
parser.add_argument(
    "-rot", "--rotate", type=float, default=0.0, help="rotate vectors angle provided"
)
parser.add_argument(
    "-di",
    "--depth_index",
...
parser.add_argument(
    ...
    nargs=2,
    type=str,
    help="second file and var to be plotted (file, epic_key)",
)
...
if args.second_var:
    ...
    nctime2 = (
        get_UDUNITS(
            EPIC2Datetime(ncdata2["time"], ncdata2["time2"]), "days since 0001-1-1"
        )
        + 1.0
    )

    ncdata2[args.second_var[1]][
        np.where(ncdata2[args.second_var[1]][:, depth_index, 0, 0] >= 1e30),
        depth_index,
        0,
        0,
    ] = np.nan
...
ncdata[args.epickey[0]][
    np.where(ncdata[args.epickey[0]][:, depth_index, 0, 0] >= 1e30), depth_index, 0, 0
] = np.nan
ncdata[args.epickey[1]][
    np.where(ncdata[args.epickey[1]][:, depth_index, 0, 0] >= 1e30), depth_index, 0, 0
] = np.nan
...
plt1, fig1 = p1.plot(
    ...
    rotate=args.rotate,
    timedata2=nctime2,
    data2=ncdata2[args.second_var[-1]],
)
...
plt1.savefig(
    "images/" + args.DataPath.split("/")[-1] + "_quiver.png",
    bbox_inches="tight",
    dpi=(300),
    transparent=False,
)
