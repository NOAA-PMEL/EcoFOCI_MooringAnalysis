...
try:
    ...
    nchandle.createVariable(
        "profileid", "S1", dimensions=("sg_data_point", "id_strlen")
    )
    nchandle.variables["profileid"].cf_role = args.add_dsg_idvar
    nchandle.variables["profileid"].long_name = "profile_id"

except:
    print(f"{args.add_dsg_idvar} - not added")

# fill with default values
profileid = args.sourcefile.split("/")[-1].split(".nc")[0]
print(profileid)
nchandle.variables["profileid"][:] = stringtochar(
    np.array(len(nchandle.dimensions["sg_data_point"]) * [profileid])
)
