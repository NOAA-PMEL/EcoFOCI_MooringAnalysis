...
parser = argparse.ArgumentParser(
    description="Calculate or update Salinity in Timeseries"
)
parser.add_argument(
...
salinity_s41, salhist = salinity_calculation(
    conductivity=ncdata["C_50"][:, 0, 0, 0],
...
if not "History" in global_atts.keys():
    ...
    nchandle.setncattr(
        ...
        "{histtime:%B %d, %Y %H:%M} UTC - {history} ".format(
            histtime=histtime, history=salhist
        ),
    )
else:
    histtime = datetime.datetime.utcnow()
