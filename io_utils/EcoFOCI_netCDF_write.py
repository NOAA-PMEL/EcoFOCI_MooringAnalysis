...
class NetCDF_BinRemoved_2D(object):
    """ Class instance to generate a NetCDF file.  
    Takes variable information from preexisting netcdf file via nchandle pass in variable_init.
    ...
    def file_create(self):
        rootgrpID = Dataset(
            self.savefile, NetCDF_BinRemoved_2D.nc_read, format=NetCDF_BinRemoved_2D.nc_format
        )
        self.rootgrpID = rootgrpID
        return rootgrpID
    ...
    def dimension_init(self, time_len=1, depth_len=1):
        ...
        Assumes
        -------
        Dimensions will be 'time', 'depth', "latitude", "longitude"
    ...
    def add_data(self, data=None, bin_index=None):
        """ """
        ...
        for ind, varname in enumerate(data.keys()):
            if not varname in [
                ...
                di = self.rec_vars.index(varname)
                self.var_class[di][:] = data[varname][:, bin_index, 0, 0]
