...
parser.add_argument(
    "gridded_file",
    metavar="gridded_file",
    type=str,
    help="full path to gridded_file file",
)
parser.add_argument("plot_var", metavar="plot_var", type=str, help="EPIC Key to plot")
parser.add_argument("depth_m", metavar="depth_m", type=int, help="local max depth")
parser.add_argument(
    "-fg",
    "--FillGaps",
    action="store_true",
...
if args.FillGaps:
    ...
    gridarray[mask] = np.interp(
        np.flatnonzero(mask), np.flatnonzero(~mask), gridarray[~mask]
    )
