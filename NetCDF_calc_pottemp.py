...
parser = argparse.ArgumentParser(description='Calculate or update Potential Temperature in Timeseries')
...
ptem_38, pothist = salinity_calculation(ncdata['S_41'][:,0,0,0],
                    ncdata['T_20'][:,0,0,0],
                    ncdata['P_1'][:,0,0,0],
                    ncdata['depth'])
...
if not 'History' in global_atts.keys():
    ...
    nchandle.setncattr('History','{histtime:%B %d, %Y %H:%M} UTC - {history} '.format(histtime=histtime,history=pothist))
else:
    histtime=datetime.datetime.utcnow()
    nchandle.setncattr('History', global_atts['History'] +'\n'+ '{histtime:%B %d, %Y %H:%M} UTC - {history}'.format(histtime=histtime,history=pothist))
