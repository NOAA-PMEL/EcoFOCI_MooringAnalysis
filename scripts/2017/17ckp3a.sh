...
serial_no=fls_028
input=${data_dir}${mooringYear}/Moorings/${mooringID}/raw/eco_fluor/17ckp3a_fls_028_41m.txt
output=${data_dir}${mooringYear}/Moorings/${mooringID}/working/${mooringID}_ecf_0041m

python ${prog_dir}EcoFOCIraw2nc.py ${input} ${output}.unqcd.nc eco 0041 -kw 0 median 0.0065 107 False -latlon $lat $lon -add_meta $mooringID $serial_no $site_depth
python ${prog_dir}EcoFOCIraw2nc.py ${input} ${output}.interpolated.nc eco 0041 -kw 3002 median 0.0065 107 True -latlon $lat $lon -add_meta $mooringID $serial_no $site_depth
python ${prog_dir}NetCDF_Time_Tools.py ${output}.interpolated.nc Trim  --trim_bounds ${deployment_date} ${recovery_date}
