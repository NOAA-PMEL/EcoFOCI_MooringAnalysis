...
serial_no=663
input=${data_dir}${mooringYear}/Moorings/${mooringID}/rawconverted/rcm/17ckip2a_rcm663.xlsx
output=${data_dir}${mooringYear}/Moorings/${mooringID}/working/17ckip2a_an9_0037m.unqcd.nc
python ${prog_dir}EcoFOCIraw2nc.py ${input} ${output} rcm9 0037 -kw True False -latlon $lat $lon -add_meta $mooringID $serial_no $site_depth
#NetCDF_Trim was combined into NetCDF_Time_Tools --> below shows example of old and new api
#python ${prog_dir}NetCDF_Trim.py ${output} -sd ${deployment_date} -ed ${recovery_date}
