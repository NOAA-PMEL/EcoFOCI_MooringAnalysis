...
serial_no=flsb_2694
input=${data_dir}${mooringYear}/Moorings/${mooringID}/raw/eco_fluor/17ckp4a_flsb_2694_42m.txt
output=${data_dir}${mooringYear}/Moorings/${mooringID}/working/${mooringID}_ecf_0039m

python ${prog_dir}EcoFOCIraw2nc.py ${input} ${output}.unqcd.nc eco 0039 -kw 0 median 0.0075 48 False -latlon $lat $lon -add_meta $mooringID $serial_no $site_depth
python ${prog_dir}EcoFOCIraw2nc.py ${input} ${output}.interpolated.nc eco 0039 -kw 1216 median 0.0075 48 True -latlon $lat $lon -add_meta $mooringID $serial_no $site_depth
python ${prog_dir}NetCDF_Time_Tools.py ${output}.interpolated.nc Trim  --trim_bounds ${deployment_date} ${recovery_date}
