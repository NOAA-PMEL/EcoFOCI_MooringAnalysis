...
python ${prog_dir}EcoFOCIraw2nc.py ${input} ${output}.interpolated.nc ecoflntu 0040 -kw 575  median 0.000 00 True -latlon $lat $lon -add_meta $mooringID $serial_no $site_depth
python ${prog_dir}NetCDF_Time_Tools.py ${output}.interpolated.nc Trim --trim_bounds ${deployment_date} ${recovery_date}
...
python ${prog_dir}EcoFOCIraw2nc.py ${input} ${output}.interpolated.cf.nc ecoflntu 0040 -kw 575  median 0.000 00 True -latlon $lat $lon -add_meta $mooringID $serial_no $site_depth -conv CF
python ${prog_dir}NetCDF_Time_Tools.py   ${output}.interpolated.cf.nc Trim  --trim_bounds ${deployment_date} ${recovery_date} --iscf --time_since_str "days since 1900-01-01T00:00:00Z"
