...
python ${prog_dir}EcoFOCIraw2nc.py ${input} ${output}.unqcd.nc ecoflntu 0039 -kw 0 median 0.0172 49 False -latlon $lat $lon -add_meta $mooringID $serial_no $site_depth
python ${prog_dir}EcoFOCIraw2nc.py ${input} ${output}.interpolated.nc ecoflntu 0039 -kw 298 median 0.0172 49 True -latlon $lat $lon -add_meta $mooringID $serial_no $site_depth
python ${prog_dir}NetCDF_Trim.py ${output}.interpolated.nc -sd ${deployment_date} -ed ${recovery_date}
