...
parser.add_argument('--time_since_str', 
        nargs='+', 
        type=str, 
        help='CF_Convert: cf compliant time since str (eg. "days since 1800-01-01"')
parser.add_argument('-is2D','--is2D', 
        action="store_true",
        help='convert files like ADCP that have two varying dimensions')
parser.add_argument('-isProfile','--isProfile', 
        action="store_true",
        help='convert files like CTD data that have one time entry')
parser.add_argument('--offset', 
        type=int, 
        help='Offset: offset in seconds if chosen as operation')
parser.add_argument('--featureType', 
        type=str, 
        help='DSG featureType - see CF standards')
parser.add_argument('--trim_bounds',
        nargs=2, 
        type=str, 
...
if (args.operation in ['CF','CF Convert','CF_Convert']) and not args.iscf:
    ...
    if args.is2D:
        ...
        ncinstance.file_create()
        ncinstance.sbeglobal_atts(raw_data_file=data_cmnt, Station_Name=data_mooring, 
                                        Water_Depth=global_atts['WATER_DEPTH'], Instrument_Type=data_insttype,
    ...
    if args.is2D:
        ...
        ncinstance = NetCDF_Copy_Struct(savefile=args.sourcefile.replace('.nc','.interp.nc'))
        ncinstance.file_create()
    ...
    if args.is2D:
        ...
        #create new netcdf file
        ncinstance = NetCDF_Trimmed(savefile=(args.sourcefile).replace('.nc','.trimmed_missing.nc'))
        ncinstance.file_create()
        ncinstance.sbeglobal_atts(raw_data_file=global_atts['DATA_CMNT'], Station_Name=global_atts['MOORING'], 
                                    Water_Depth=global_atts['WATER_DEPTH'], Instrument_Type=global_atts['INST_TYPE'],
                                    Water_Mass=global_atts['WATER_MASS'], Experiment=global_atts['EXPERIMENT'],
